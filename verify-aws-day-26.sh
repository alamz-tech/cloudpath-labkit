#!/usr/bin/env bash
# CloudPath — aws-day-26 verification.
# Usage:  bash verify-aws-day-26.sh <CODE>
set -euo pipefail
NONCE="${1:?Paste your code like:  bash verify-aws-day-26.sh ABC234}"
LAB_ID="aws-day-26"
SECRET="__injected_by_deploy__"   # server injects GRADING_HMAC_SECRET
FILE="$HOME/cloudpath/budget.json"
...
lim = d.get("BudgetLimit", {})
amount = str(lim.get("Amount", "")).strip() in ("20", "20.0")
monthly = str(d.get("TimeUnit","")).upper() == "MONTHLY"
thr = float(d.get("NotificationThresholdPercent", -1)) == 50
print(f"1 {int(amount)} {int(monthly)} {int(thr)}")
PY
)"
fi
